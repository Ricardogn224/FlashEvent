...
func RegisterAuthRoutes(router *mux.Router, api *swag.API, db *gorm.DB) {
	api.AddEndpoint(
		...
		endpoint.New(
			http.MethodPatch, "/participant-present/{participantId}",
		...
		endpoint.New(
			http.MethodPatch, "/participant-contribution/{participantId}",
			endpoint.Handler(http.HandlerFunc(controllers.UpdateParticipantContribution(db))),
			endpoint.Summary("Modifier un participant"),
			endpoint.Description("Modifier participant d'un événement"),
			endpoint.Path("participantId", "integer", "ID du particpant", true),
			endpoint.Body(models.Participant{}, "Data Participant", true),
		...
		endpoint.New(
			http.MethodGet, "/participants-contribution/{eventId}",
			endpoint.Handler(http.HandlerFunc(controllers.GetParticipantsByEventIDWithContribution(db))),
			endpoint.Summary("Get participants by event ID"),
			endpoint.Description("Retrieve participants associated with a specific event"),
			endpoint.Path("eventId", "integer", "ID of the event", true),
			endpoint.Response(http.StatusOK, "Successfully retrieved participants", endpoint.SchemaResponseOption(&[]models.Participant{})),
		...
		endpoint.New(
			http.MethodPost, "/events/{eventId}/cagnottes",
		...
		endpoint.New(
			http.MethodGet, "/events/{eventId}/cagnotte",
			endpoint.Handler(controllers.AuthMiddleware(controllers.GetCagnotteByEventID(db))),
			endpoint.Summary("Voir la cagnotte d'un événement"),
			endpoint.Description("Voir la cagnotte associée à un événement"),
			endpoint.Path("eventId", "integer", "ID de l'événement", true),
			endpoint.Response(http.StatusOK, "Cagnotte récupérée avec succès", endpoint.SchemaResponseOption(&models.Cagnotte{})),
			endpoint.Security("BearerAuth"),
			endpoint.Tags("Cagnotte"),
		),
		endpoint.New(
			http.MethodGet, "/features/transport",
			endpoint.Handler(http.HandlerFunc(controllers.FindTransportFeatureHandler(db))),
			endpoint.Summary("Find transport feature"),
			endpoint.Description("Find and return the feature with the name 'transport'"),
			endpoint.Response(http.StatusOK, "Successfully retrieved transport feature", endpoint.SchemaResponseOption(&models.Feature{})),
